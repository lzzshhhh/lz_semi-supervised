...
class Pancreas(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h5f = h5py.File(self._base_dir + 'PANCREAS_'+image_name + ".h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
...
#     def __call__(self, sample):
#         image = sample['image']
#         image = torch.from_numpy(image.reshape(
#             1, image.shape[0], image.shape[1], image.shape[2]).float())
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        image = torch.from_numpy(image[None, ...]).float()  # 添加 channel 维度
